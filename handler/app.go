...
func CreateVersion(db *xorm.Engine, params martini.Params, version model.Version, r render.Render, client redis.Client, res *http.Request, logChan *chan string, sys_log_level string) {
	...
	logId, fromModel := getHeader(res)
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]开始检验http中的json数据是否完整 UPDATE:handler.CreateVersion LOG_START", fromModel, logId)

	if version.VersionId == 0 || version.Name == "" || version.Changed == "" || version.Url == "" || version.Client == "" || version.Compatible == "" || version.Updated == "" {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Invalid json body UPDATE:handler.CreateVersion LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Invalid json body "})
		return
	}
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]json数据完整验证完成,查询此App的版本号是否已经存在 UPDATE:handler.CreateVersion", fromModel, logId)
	...
	if has && errDb == nil {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]The application's id already exist UPDATE:handler.CreateVersion LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "The application's id already exist"})
		return
	} else {
		logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]App未注册版本，开始进行注册 UPDATE:handler.CreateVersion", fromModel, logId)
		version.App = appId
		_, err2 := db.Insert(version)
		if err2 != nil {
			logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Database Insert error UPDATE:handler.CreateVersion LINE:65 LOG_END", fromModel, logId)
			r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Database error"})
			return
		} else {
			logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]注册成功，插入redis  UPDATE:handler.CreateVersion", fromModel, logId)
			//写入redis内存库
			...
			logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]插入成功，处理返回值  UPDATE:handler.CreateVersion LOG_END", fromModel, logId)
...
func GetVersion(db *xorm.Engine, params martini.Params, r render.Render, client redis.Client, res *http.Request, logChan *chan string, sys_log_level string) {
	...
	logId, fromModel := getHeader(res)
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]开始检验http中的Header数据是否完整 UPDATE:handler.CreateVersion LOG_START", fromModel, logId)

	_, id := res.Header["X-Arkors-Application-Id"]
	_, Token := res.Header["X-Arkors-Application-Token"]
	if id != true || Token != true {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Invalid request header,it should be include 'id' and 'Token' UPDATE:handler.GetVersion LINE:112 LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Invalid request header,it should be include 'id' and 'Token'"})
		return
	}
	//获取内存库中所有的key值
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]从redis中读取数据 UPDATE:handler.GetVersion", fromModel, logId)
	keyAll, redisErr := client.Keys(params["app"] + "@*")
	if redisErr != nil {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]redis error UPDATE:handler.GetVersion LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "redis error"})
		return
	}
	//内存库key值不存在，查询mysql确认是否存在，如果存在，把数据重新插入到redis中
	if keyAll == nil {
		logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]内存库key值不存在，查询mysql确认是否存在，如果存在，把数据重新插入到redis中 UPDATE:handler.GetVersion", fromModel, logId)
		...
		if errDb == nil && has {
			...
			logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]把数据重新插入到redis UPDATE:handler.GetVersion LOG_END", fromModel, logId)
	...
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]获取最新版本信息 UPDATE:handler.GetVersion", fromModel, logId)
	...
	if err == nil && result != nil {
		...
		if json2versionErr != nil {
			logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]json trans struct error UPDATE:handler.GetVersion LOG_END", fromModel, logId)
	...
	if upgrade {
		...
		if err != nil {
			logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Prase Time Error UPDATE:handler.GetVersion LOG_END", fromModel, logId)
		...
		if timeResult {
			...
			logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]The App version can't in right time to upgrade UPDATE:handler.GetVersion LOG_END", fromModel, logId)
			r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "The App version can't in right time to upgrade"})
			return
		}
	} else {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]The App version can't upgrade UPDATE:handler.GetVersion LOG_END", fromModel, logId)
...
func UpdateVersion(db *xorm.Engine, params martini.Params, version model.Version, r render.Render, client redis.Client, res *http.Request, logChan *chan string, sys_log_level string) {
	...
	logId, fromModel := getHeader(res)
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]开始检验http中的Header数据是否完整 UPDATE:handler.UpdateVersion LOG_START", fromModel, logId)
	if version.VersionId == 0 || version.Name == "" || version.Changed == "" || version.Url == "" || version.Client == "" || version.Compatible == "" {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Invalid json body UPDATE:handler.UpdateVersion LINE:261 LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Invalid json body "})
		return
	}
	version.App = appId
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]开始检验App是否存在 UPDATE:handler.UpdateVersion", fromModel, logId)
	has, err := db.Where("app=?", appId).And("version_id=?", version.VersionId).Update(version)
	if err != nil {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Database Error  UPDATE:handler.UpdateVersion LOG_END", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Database Error"})
		return
	}
	if has == 0 {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Not found any version records UPDATE:handler.UpdateVersion LINE:275", fromModel, logId)
...
func DelVersion(db *xorm.Engine, params martini.Params, r render.Render, client redis.Client, res *http.Request, logChan *chan string, sys_log_level string) {
	...
	logId, fromModel := getHeader(res)
	logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]开始检验删除的APP是否存在 UPDATE:handler.DelVersion LOG_START LINE:275", fromModel, logId)
	result := new(model.Version)
	result.App = appId
	result.VersionId = versionNumber
	has, err := db.Where("app=?", appId).And("version_id=?", versionNumber).Get(result)
	if err != nil {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Database Error UPDATE:handler.DelVersion", fromModel, logId)
		r.JSON(http.StatusBadRequest, map[string]interface{}{"error": "Datebase Error"})
		return
	}
	if has {
		logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]查到记录，开始删除 UPDATE:handler.DelVersion", fromModel, logId)
		...
		if affect == 1 && err == nil {
			logId = logmodel.WriteLog(logChan, sys_log_level, "DEBUG", appId, "[normal]删除成功，处理返回值 UPDATE:handler.DelVersion LOG_END", fromModel, logId)
			client.Del(params["app"] + "@" + params["version"])
			r.JSON(http.StatusOK, result)
			return
		}
	} else {
		logId = logmodel.WriteLog(logChan, sys_log_level, "ERROR", appId, "[error]Application's ID is not exist! UPDATE:handler.DelVersion LOG_END", fromModel, logId)
